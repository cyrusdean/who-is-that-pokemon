...
const finishGame = ()=>{
    ...
    const { generations, selectedGenerations, pokemon: initialPokemon, correctGuessesStreak, ...restInitialState } = initialState;
    state = {
        ...state,
        ...restInitialState
    };
...
const beginGuessing = ()=>{
    ...
    ].map(()=>pokemon[randomNumUpTo(pokemonIndexMax)]);
...
const setupRandomPokemon = ()=>{
    const selectedPokemon = state.pokemon[randomNumUpTo(state.pokemon.length - 1)];
...
const init = async ()=>{
    ...
    const { data: { generations } } = pokemonAPIResponse;
    state.generations = generations.reduce((a, { name, pokemon_species: pokemon })=>({
            ...a,
    ...
    let pokemon = [].concat(...state.selectedGenerations.map((selectedGeneration)=>state.generations[selectedGeneration]));
    ...
    generationNames.forEach((generationName, i)=>{
        ...
        generationOptionEl.addEventListener("click", (e)=>{
            ...
            state.selectedGenerations = isRemoving ? state.selectedGenerations.filter((selectedGeneration)=>selectedGeneration !== generationName) : [
                ...state.selectedGenerations,
                generationName
            ];
            state.pokemon = [].concat(...state.selectedGenerations.map((generationName)=>state.generations[generationName]));
...
},{}]},["km5uZ","bB7Pu"], "bB7Pu", "parcelRequireb2f8")
