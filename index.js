...
const pokemonApp = {
  ...
  init: async function () {
    ...
    pokemon = pokemon.map(({ name }, i) => {
      ...
      const avatarUrl = `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/dream-world/${
        i + 1
      }.svg`;
      return {
  ...
  clearHTML: function () {
    ...
    const whoIsThatPokemonBannerEl = document.getElementById(
  ...
  startGame: function () {
    ...
    const selectedPokemonIndex = utils.randomNumUpTo(
  ...
  beginGuessing: function () {
    ...
    let guessingOptionIndexes = [...Array(numOfOtherOptions)].map(() =>
      randomNumUpTo(pokemonIndexMax)
    );
    console.log(guessingOptionIndexes);
    guessingOptionIndexes.splice(
      randomNumUpTo(numOfOtherOptions),
      0,
  ...
  finishGame: function () {
    ...
    const guessedCorrectly =
      selectedPokemonIndex === guessedPokemonIndex && !didNotGuess;
    let textResult = guessedCorrectly ? "Yep" : "Nope";
    textResult = didNotGuess ? "Out of time" : textResult;
    ...
    const [correctGuessesEl, incorrectGuessesEl] =
